...
export default function({
	...
	const onClick = async (e) => {
		...
		console.table({ dataAction, selectedIdRef })
...
export const Selector = ({ name, options, isDisabled = false, placeholder, onChange, value, ...props }) => {
	...
	return (
		...
		>
			<SelectTrigger className="rounded text-muted-foreground bg-white font-normal text-[#777]">
			...
			<SelectContent className="bg-white text-muted-foreground">
				{!!options && options?.map(option => (
					<SelectItem
						className="hover:cursor text-[#777]"
						key={option.id}
